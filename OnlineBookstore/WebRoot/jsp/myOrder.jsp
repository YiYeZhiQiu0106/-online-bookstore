...
<script>
   function delOrd(o_id){
	     $.ajax({
	    	 url:"${pageContext.request.contextPath}/shopCat/delOrd.action?o_id="+o_id,
	    	 dataType:"json",
	    	 type:"post",
	    	 success:function(t){
	    		 alert(t.ord);
	    		 window.location.href="${pageContext.request.contextPath}/shopCat/getOrders.action?u_id=${user.id}";
	    	 }
...
<ul class="breadcrumbs">
 <div class="container">
     <li class="home">
         <span class="red" style="font-size: 20px"> &nbsp;我的订单&nbsp;</span>
     </li>
 </div>
</ul>
<div class="contact">
    <div class="container">
     <div class="contact_top">
      	  <table  align="left"  id="order" class="table">
      	  <tr align="center">
      	    <td width="200px" >详情</td>
      	    <td width="80px">购买数量</td>
      	    <td width="80px">支付金额</td>
      	    <td width="80px">发货状态</td>
      	    <td width="100px">操作</td>
      	  </tr>
      <c:forEach items="${orderlist}" var="order" varStatus="st" >
       <tr align="center">
          <td height="70px"><a href="${pageContext.request.contextPath}/book/getOneBook.action?b_id=${order.b_id}"><label >${order.b_name}   价格：${order.price} </label></a></td>
          <td>${order.num }</td>
         <td>${order.money}</td>
         <td><c:if test="${order.o_state==1}"><h3 style="color: #00FF00">已发货</h3></c:if><c:if test="${order.o_state==0}"><h3 style="color:red">未发货</h3></c:if></td>
         <td><c:if test="${order.o_state==0}"><button class="btn btn-info" onclick="delOrd(${order.o_id})">取消订单</button></c:if><c:if test="${order.o_state==0}"></c:if></td>
        </tr>
        </c:forEach> 
