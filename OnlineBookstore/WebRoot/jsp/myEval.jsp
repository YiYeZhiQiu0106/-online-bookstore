...
<script type="text/javascript">
			    ...
			    function delEva(e_id,b_id){
			    	var id=${user.id};
					   $.ajax({
						   url:"${pageContext.request.contextPath}/book/delEva.action",
						   data:{"e_id":e_id,"u_id":id,"b_id":b_id},
						   type:"POST",
						   dataType:"json",
						   success:function(data){
							   alert(data.eva);
							   window.location.href="${pageContext.request.contextPath}/book/getUserEva.action?u_id="+id;
						   }
					   })
			    }
</script>	
...
<ul class="breadcrumbs">
 <div class="container">
     <li class="home">
         <span class="red" style="font-size: 20px"> &nbsp;我的评论&nbsp;</span>
     </li>
 </div>
</ul>
<div class="contact">
    <div class="container">
     <div class="contact_top">
      	  <table  align="left"  id="eva" class="table">
      	  <tr align="center">
      	    <td width="150px" >图书</td>
      	    <td width="300px">评论</td>
      	    <td width="80px">评论时间</td>
      	    <td width="80px">状态</td>
      	    <td width="100px">操作</td>
      	  </tr>
      <c:forEach items="${evaUser}" var="eva" varStatus="st" >
       <tr align="center">
          <td height="70px"><a href="${pageContext.request.contextPath}/book/getOneBook.action?b_id=${eva.b_id}"><label >${eva.b_name}  ${eva.author} </label></a></td>
         <td>${eva.evaluate }</td>
         <td>${eva.e_time}</td>
         <td>
         <c:if test="${eva.statis==1}"><font color="#00FF00">已通过审核</font></c:if>
         <c:if test="${eva.statis==2}"><font color="orange" >审核中</font><text></c:if>
         <c:if test="${eva.statis==3}"><font color="red">未通过审核</font></c:if>
         </td>
         <td><button class="btn btn-info" onclick="delEva(${eva.e_id},${eva.b_id})">删除评论</button></td>
        </tr>
        </c:forEach> 
