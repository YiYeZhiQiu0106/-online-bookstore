...
<script type="text/javascript">
	...
	$(function(){
		...
		url:"${pageContext.request.contextPath}/b_eva/getEvaluates.action?sta=3",
		async:false,
		idField:'e_id',//标识字段
			...
			if(row){
				...
				$.messager.confirm("Confirm","确定通过审核吗?",function(result){
					if(result){
						...
						$.post("${pageContext.request.contextPath}/b_eva/upEvaSta.action",{e_id:row.e_id,sta:2},
							function(result){
								if(result.eva=="操作成功"){
						 			$.messager.show({'title':'error','msg':"通过审核成功"});
									//刷新表格   重新请求后台
									$("#dg").datagrid('reload');
								}else{
